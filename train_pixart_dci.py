...
def main():
    ...
    def preprocess_train(examples):
        image_paths = [os.path.join(args.train_data_dir, image) for image in examples[image_column]]
    ...
    CAPTION_PATH = "dci/T5/captions/"
    def collate_fn(examples):
        images = [example["image_paths"] for example in examples]
        ...
        for example in examples:
            l = torch.load(os.path.join(CAPTION_PATH, dataset_name, example[image_column]+'.pt'))
            text_embedding.append(l['text_embedding'])
            prompt_attention_mask.append(l['attention_mask'])
        text_embedding = torch.cat(text_embedding, dim=0)
        prompt_attention_mask = torch.cat(prompt_attention_mask, dim=0)
        return {"pixel_values": pixel_values, "text_embedding": text_embedding, 'prompt_attention_mask': prompt_attention_mask}
    ...
    transformer, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(transformer, optimizer,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                prompt_embeds = batch['text_embedding'].to(dtype=weight_dtype)
        ...
        # if accelerator.is_main_process:
            ...
            #             tracker.log(
            #                 {
            #                     "validation": [wandb.Image(image, caption=f"{i}: {args.validation_prompt}") for i, image
            #                                    in enumerate(images)]
            #                 }
            #             )
            #
    ...
    # pipeline = DiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path, transformer=transformer,
    #                                              text_encoder=text_encoder, vae=vae, torch_dtype=weight_dtype, )
    # pipeline = pipeline.to(accelerator.device)
    #
    # del transformer
    # torch.cuda.empty_cache()
